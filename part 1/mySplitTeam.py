  ...
  def chooseAction(self, gameState):
    ...
    for i in xrange(-2,3):
        if x+i <= 0 or x+i >= gameState.data.layout.width:
            continue
        elif not gameState.data.layout.walls[x+i][y]:
            self.inCave = False
            break
    for j in xrange(-2,3):
        if y+j <= 0 or y+j >= gameState.data.layout.height:
            continue
        elif not gameState.data.layout.walls[x][y+j]:
            self.inCave = False
            break
    '''
...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
      ...
      if myFood.__class__ == tuple:
          ...
          if self.firstDot not in foodList and self.getMazeDistance(myPos, friendPos) <= 2 * radius:
             ...
             # print 'Agent', self.index, "friend within radius of", radius
              ...
              # Then find a set of dots that are good candidates for next dot to target
              # generate the set by finding all dots within the radius
              # friendDot = closest to friend, mySet = set of dots within my radius
              # myDot = the point in mySet such that the maze_distance between myDot and friendDot is maximized
              # Corner Case: steps from closest agent to next dot(s) triggers return home,
              ## far agent should return home
    ...
    '''
      ...
      lastD = gameState.getAgentState(self.index).configuration.direction
      if action == lastD : features['sameD'] = 1
  ...
  def getWeights(self, gameState, action):
    return {'successorScore': 100, 'distanceToFood': -1, 'stop': -500, 'reverse': -1, 'sameD': .5}#'inQuad': -.25} #500 fails for alley and office
