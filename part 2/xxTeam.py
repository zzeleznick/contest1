...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def minValues(self, state, action, minAgentId, depth):
        if depth > self.depth:
        #if state.isWin() or state.isLose() or depth > self.depth:
            return self.evaluate(state, action, self.index)
        elif self.getMazeDistance(state.getAgentPosition(self.index), state.getAgentPosition(minAgentId)) == 0:
            return -99999
  ...
  def chooseAction(self, gameState):
    ...
    try:
        values = [self.maxValues(gameState, a, minAgentId, 3, FIRST_CALL = True) for a in actions]
    except:
        print "shit, we did something illegal"
        values = [0 for a in actions]
...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action, MM_ID = False): #MM_ID: IF eval passed from gamestate
    ...
    if len(capsules) > 0:
        ...
        capsuleProgress = 1
    oldCapsules = self.getCapsules(self.getCurrentObservation())
    if capsuleProgress:
        pass
    if len(capsules) < len(oldCapsules):
        capsuleProgress = 200
    features['capsuleScore'] = capsuleProgress
...
class DefensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action, MM_ID = False):
    features = util.Counter()
    if  MM_ID or MM_ID == self.index:
        #print "Eval Passed from Minimax"
        #successor = gameState.generateSuccessor(EID, action)
