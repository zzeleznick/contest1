...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    '''
    if self.getState() == 'OFFENSE' and self.bestFood not in foodLeft:
        enemies = [gameState.getAgentState(i) for i in self.getOpponents(gameState)]
        protectors = [a for a in enemies if a.getPosition() != None and not a.isPacman]
        if len(protectors) > 0:
            dists = [(self.getMazeDistance(pos, a.getPosition()), a) for a in protectors]
            closestDist, closestGhost = min(dists)
            bestLead = -9999
            for food in foodLeft:
               myDist = self.getMazeDistance(pos, food)
...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action, MM_ID = False): #MM_ID: IF eval passed from gamestate
    ...
    flag = len(foodList) > 0
    '''
    if self.bestFood in foodList:
        distToFood = self.getMazeDistance(myPos, self.bestFood)
        if self.getMazeDistance(closestGhost.getPosition(), self.bestFood) >  distToFood:
            flag = False
        preFoodDist = self.getMazeDistance(lastPos, self.bestFood)
        foodProgress = distToFood < preFoodDist
        features['splitDistToFood'] = -distToFood
    '''
    if flag: # This should always be True,  but better safe than sorry
      ...
      elif bestFood != self.bestFood and action != Directions.STOP:
          print "Food Changed from ", self.bestFood, 'to ', bestFood
          progress = self.getMazeDistance(lastPos, bestFood) < self.getMazeDistance(myPos, bestFood)
          oldDist =  self.getMazeDistance(lastPos, self.bestFood)
          features['splitDistToFood'] = -oldDist + int(progress)
          self.bestFood = bestFood
    ...
    if not MM_ID or MM_ID == 7:  #successor state is same as gamestate
        ...
        '''
        if len(successor.getLegalActions(self.index)) == 2:  #TODO FIX WEIGHT FOR TRAPPING
